...
from outsource.models import KindChoice, LanguageChoice, Projects, News, PublishProject
...
def projects_detail(request, projects_id):
    return render(request, 'outsource/projects_detail.html')
...
def publish(request):
    if request.method == 'GET':
        ...
        PublishProject.objects.create(project_name=project_name, kind=kind, budget=budget, language=languages,
                                      cycles=cycles, project_desc=project_desc, user_id=passport_id)
        return render(request, 'outsource/publish.html', locals())
